...
SELECT 
	TO_CHAR(CS.CAR_SELL_DATE,'YYYY') AS SELL_YEAR,
	CM.CAR_MAKER_NAME, SUM(CS.CAR_SELL_EA) AS TOTAL_SELL_EA,
	SUM(CS.CAR_SELL_PRICE) AS TOTAL_SELL_PRICE,
	TRUNC(AVG(CS.CAR_SELL_PRICE),0) AS TOTAL_SELL_PRICE
FROM CAR C JOIN CAR_SELL CS ON C.CAR_ID = CS.CAR_ID
JOIN CAR_MAKER CM ON C.CAR_MARKER_CODE = CM.CAR_MARKER_CODE
GROUP BY TO_CHAR(CS.CAR_SELL_DATE,'YYYY'),CM.CAR_MAKER_NAME;
--판매 연도/월별, 제조사별, 판매 대수 총합, 판매금액 총합, 판매금액 평균을 조회
SELECT 
	TO_CHAR(CS.CAR_SELL_DATE,'YYYY/MM') AS SELL_YEAR,
	CM.CAR_MAKER_NAME, SUM(CS.CAR_SELL_EA) AS TOTAL_SELL_EA,
	SUM(CS.CAR_SELL_PRICE) AS TOTAL_SELL_PRICE,
	TRUNC(AVG(CS.CAR_SELL_PRICE),0) AS TOTAL_SELL_PRICE
FROM CAR C JOIN CAR_SELL CS ON C.CAR_ID = CS.CAR_ID
...
SELECT 
	TO_CHAR(CS.CAR_SELL_DATE,'YYYY"년도 "Q"분기"') AS SELL_YEAR,
